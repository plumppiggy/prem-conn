...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for x_i in self.domains[x]:
            overlapping_char = x_i[overlap[0]]
            y_chars = [y_i[overlap[1]] for y_i in self.domains[y]]

            if overlapping_char not in y_chars:
                to_remove.add(x_i)
                removed = True
    ...
    def assignment_complete(self, assignment):
        ...
        """
        return set(assignment.keys()) == self.crossword.variables and all(assignment[var] is not None for var in assignment)
        ...
        for var, word in assignment.items():
            ...
            for neighbour in self.crossword.neighbors(var):
                if neighbour in assignment:
                    ...
                    if word[overlap[0]] != assignment[neighbour][overlap[1]]:
                        return False
        return True
    ...
    def select_unassigned_variable(self, assignment):
        ...
        num_remaining = {var: len(self.domains[var]) for var in unassigned_vars}
        sorted_num_rem = sorted(num_remaining.items(), key=lambda x: x[1])

        if len(sorted_num_rem) == 1 or sorted_num_rem[0][1] != sorted_num_rem[1][1]:
            # no tie
            return sorted_num_rem[0][0]
    ...
    def backtrack(self, assignment):
        ...
        for value in self.order_domain_values(var, assignment):
            assn_copy = copy.deepcopy(assignment)
            assn_copy[var] = value
            if self.consistent(assn_copy):
...
def main():
    ...
    if assignment is None:
        ...
        print(creator.letter_grid(assignment))
